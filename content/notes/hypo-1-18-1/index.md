...
{{< img src="screenshot-import.webp" alt="Hypo import confirmation sheet with six photos, destination folder and free space" >}}

Hypo's job is to copy files, and it used to offer deleting the original as soon as the write didn't throw an internal error, and if you said yes, it would do it. I learned a copy can come back incomplete without anything throwing an error, yet it can count as imported. The new safety net is that every copy gets read back and compared with both source and destination before anything is deleted. That roughly doubles the copy time but it is still fast enough for large imports. There are a few more improvements around duplicate checks, RAW+JPEG pair handling and deletion from the built-in SD card slot.

A few things I changed my mind about and want to be able to laugh about later:
...
I did a color refresh of the UI to be more pleasant to my eyes and I implemented patch releases. So more 1.x.y releases than always bumping a minor version with 100 changes cramped together.
