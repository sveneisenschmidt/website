...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const buildAsciiGraph = (data, container) => {
        const colWidth = 32;
        const containerWidth = container.offsetWidth;
        const maxDays = Math.min(21, Math.max(3, Math.floor(containerWidth / colWidth)));
        const days = [];
    ...
    Promise.all([
        ...
        const graph = buildAsciiGraph(daily, container);
        container.innerHTML = `<h4>Statistics</h4>
